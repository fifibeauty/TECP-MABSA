...
# huggingface_hub.login(token='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',add_to_git_credential=False)

# from stanfordcorenlp import StanfordCoreNLP
# nlp = StanfordCoreNLP('D:/project/StanfordParser/stanford-corenlp-4.5.4/stanford-corenlp-4.5.4')
...
class TrainInputProcess:
    def __init__(self, 
        ...
        self.data_anp_dir = data_anp_dir
        self.aux_img_dir = aux_img_dir
        self.img_region_dir = region_dir
        self.cap_dir = cap_dir
        self.cap_weight_dir = cap_weight_dir
    ...
    def get_img_region(self, id, mode):
        region_feat = np.load(
            os.path.join(self.img_region_dir+mode+ '/_att',
                         id[:-4] + '.npz'))['feat']

        return region_feat

    def generate_dualc_input(self):
        anp_data = json.load(open(self.data_anp_dir + '/image_output2015.json', 'r'), )
        caption_data = json.load(open(self.cap_dir + '/twitter2015_images.json','r'),)
        caption_weight_data = json.load(open(self.cap_weight_dir + '/twitter2015_images.json', 'r'), )
        for dataset_type in self.dataset_types:
            sentence_l, image_l, label_l, pair_l = self.data_dict[dataset_type]
            images = []
            anps = []
            ...
            for image_path in image_l:
                ...
                # region_feat = self.get_img_region(image_path,dataset_type)
                # # region_feat = torch.tensor(region_feat)
                # region_feats.append(region_feat)

                caption = caption_data[image_path].split(' ')
                captions.append(caption)
                weighted_captions.append(caption_weight_data[image_path])

                # if self.aux_img_dir is not None:
                #     aux_img_name = image_path.split('.')[0]
                #     path = "{}/{}.pth".format(self.aux_img_dir, aux_img_name)
                #     aux_img = torch.load(path)
                #     aux_img = torch.stack(aux_img , dim=0)
                #     aux_imgs.append(aux_img.cpu().numpy())

                anp_list = anp_data[image_path]
                anp_list = dict(sorted(anp_list.items()))
                anp_value = [0] * len(anp_list)
                for i, (k, v) in enumerate(anp_list.items()):
                    anp_value[i] = v
                # anp_value = torch.tensor(anp_value)
                anps.append(anp_value)


                # for caption in captions:
                #     sentence = "".join(caption)
                #     parsed_result = nlp.dependency_parse(sentence)
                #     parsed_captions.append(parsed_result)
                #
                # for dependency_tree in parsed_captions:
                #     weighted_caption = []
                #     for i, token in enumerate(dependency_tree):
                #         head_index = token[1]
                #         if head_index >= 0 and head_index < len(dependency_tree):
                #             if i == head_index or head_index in range(i - 2, i + 2):
                #                 weighted_caption.append(1)  # 其他词汇赋子权重 0
                #             else:
                #                 weighted_caption.append(0.5)  # 父点索引无效时，赋予权重 0weighted_captions.append(weighted_caption)
                #         else:
                #             weighted_caption.append(0.5)  # 父点索引无效时，赋予权重 0weighted_captions.append(weighted_caption)
                #     weighted_captions.append(weighted_caption)

                    # #aux_imgs = torch.tensor(aux_imgs)
            anps = torch.tensor(anps)
            # break
            ...
            # tokenized_inputs_0 = self.tokenizer(sentence_l[0], truncation=True, is_split_into_words=True,
            #                                   padding='max_length', max_length=60, return_tensors='pt')
            # clip_tokenized_inputs = clip_tokenizer(new_sentence_l, truncation=True, padding='max_length', max_length=60, return_tensors='pt')

            cap_tokenized_inputs = self.tokenizer(captions, truncation=True, is_split_into_words=True, padding='max_length', max_length=26, return_tensors='pt')
            ...
            for i, label in enumerate(label_l):
                ...
                text_labels.append(label_ids)
            max_length = 26
            for i in range(len(weighted_captions)):
                if len(weighted_captions[i]) < max_length:
                    weighted_captions[i] += [0] * ( max_length - len(weighted_captions[i]))
                if len(weighted_captions[i]) > max_length:
                    weighted_captions[i] = weighted_captions[i][:max_length]
            # tokenized_inputs["attention_weights"] = torch.tensor(weighted_captions)
            # tokenized_inputs["labels"] = torch.tensor(text_labels)
            # tokenized_inputs["pairs"] = pair_l
            # tokenized_inputs["cross_labels"] = torch.tensor(cross_labels)
            # tokenized_inputs["pixel_values"] = pixel_values
            # # tokenized_inputs["aux_pixel_values"] = aux_imgs
            # tokenized_inputs["anp_labels"] = anps
            # # tokenized_inputs["region_feats"] = region_feats
            # tokenized_inputs["cap_input_ids"] = cap_tokenized_inputs['input_ids']
            # # tokenized_inputs["cap_attention_mask"] = cap_tokenized_inputs['attention_mask']
            # # tokenized_inputs["clip_input_ids"] = clip_tokenized_inputs["input_ids"]
            # tokenized_inputs = tokenized_inputs_0
            tokenized_inputs["attention_weights"] = torch.tensor(weighted_captions)
            tokenized_inputs["labels"] = torch.tensor(text_labels)
            tokenized_inputs["pairs"] = pair_l
            tokenized_inputs["cross_labels"] = torch.tensor(cross_labels)
...
def main():
    ...
    # inputs output dir
    parser.add_argument('--output_dir', type=str, default='D:/pycharm/tecp/utils/datasets/finetune/smaple', nargs='?', help='display an string')
    ...
    if text_model_type == 'bert':
        ...
        text_model = 'D:/pycharm/pretrained/roberta-base-cased'

    if image_model_type == 'vit':
        image_model = 'D:/pycharm/pretrained/vit-base-patch16-224-in21k'
        # image_model = 'D:/project/pretrain_model/vit-base-patch32-224-in21k'
    ...
    if dataset_type == '2015':
        data_text_dir = 'D:/pycharm/data/IJCAI2019_data/twitter2015'
        data_image_dir = '../../data/IJCAI2019_data/twitter2015_images'
        data_anp_dir= 'D:/pycharm/data/IJCAI2019_data/ANP_data'
        aux_img_dir= 'D:/pycharm/data/aux_img2015'
        region_dir = 'D:/pycharm/data/IJCAI2019_data/region_box/15/'
        # cap_dir = 'D:/project/data/captions'
        cap_dir = 'D:/pycharm/data/caption_lavis'
        # cap_dir = 'D:/project/data/caption_cut'
        # cap_dir = 'D:/project/data/caption_detcut'
        cap_weight_dir = 'D:/pycharm/data/caption_lavis_weight'
    elif dataset_type == '2017':
        data_text_dir = '../../data/IJCAI2019_data/twitter2017'
        data_image_dir = '../../data/IJCAI2019_data/twitter2017_images'
        data_anp_dir= 'D:/pycharm/data/IJCAI2019_data/ANP_data'
        aux_img_dir= 'D:/pycharm/data/aux_img2017'
        region_dir = 'D:/pycharm/data/IJCAI2019_data/region_box/17/'
        # cap_dir = 'D:/project/data/captions'
        cap_dir = 'D:/pycharm/data/caption_lavis'
        # cap_dir = 'D:/project/data/caption_cut'
        # cap_dir = 'D:/project/data/caption_detcut'
    ...
    trainInputProcess = TrainInputProcess(text_model,
                                  ...
                                  data_anp_dir=data_anp_dir,
                                  aux_img_dir=aux_img_dir,
                                  region_dir=region_dir,
                                  cap_dir=cap_dir,
                                  cap_weight_dir=cap_weight_dir,
